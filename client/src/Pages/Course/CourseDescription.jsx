...
const VideoLabels = ({
	...
	async function getVideoProgress() {
		...
		try {
			const response = await axiosInstance.get(
				`youtube/playlist/video/progress/?id=${url}&videoId=${videoURL}&email=${email}`,
	...
	async function progressHandler(e) {
		...
		try {
			...
			const response = await axiosInstance.patch(
				`youtube/playlist/video/progress`,
				body
			);
			callGetProgress();
	...
	return (
		<div
			className={`h-[3.5rem] flex items-center justify-between ${
				checked ? `bg-lime-700` : `bg-black`
			} text-white my-2 px-2 shadow-[0px_0px_2px_lime] rounded-sm`}
		>
			<p
				className="overflow-hidden whitespace-nowrap text-ellipsis font-semibold mr-2 cursor-pointer min-h-full py-[10%]"
...
export default function CourseDescription() {
	...
	const getProgress = async () => {
		...
		try {
			const response = await axiosInstance.get(
				`youtube/playlist/progress/?id=${data.playlist_url}&email=${email}`
			);
			console.log(response.data);
	...
	return (
		<HomeLayout>
			...
			<main className="flex lg:h-[80svh] gap-3 relative max-lg:flex-wrap max-w-full">
				<progress
					className="w-full h-[2svh] absolute top--2 customProgress"
				...
				<div className="min-w-[65svw] min-h-full flex flex-col justify-center p-2 px-5 relative grow">
					...
					<div className="flex justify-center gap-[50px] max-lg:mt-3">
						<button
							className="bg-black px-2 rounded-md text-yellow-500 text-xl cursor-pointer"
							onClick={handlePrev}
						>
							Prev
						</button>
				...
				<aside className="min-h-full px-2 lg:overflow-y-auto lg:border-l  border-lime-300 max-lg:max-w-[100svw] grow">
					{allVideos.map((v, index) => (
						<VideoLabels
							...
							handleOnCLick={changeVideo}
							callGetProgress={getProgress}
						/>
					))}
				</aside>
			</main>
