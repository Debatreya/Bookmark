...
function GetCourse(){
    ...
    function createPlaylist(e){
        ...
        console.log(body);
        axiosInstance.post("youtube/playlist", body, { withCredentials: true })
        .then((response) => {
    ...
    return (
        <div className="m-5 flex flex-col gap-1 justify-center items-center shadow-md shadow-white p-5 max-w-[30svw] min-w-[25svw] min-h-[60svh] border border-yellow-600 rounded-lg">
            ...
            />
            <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={createPlaylist}>
                CREATE
            </button>
        </div>
    )
...
// Kaam nhi kr rha dispatch se
    ...
    async function loadCourses(){
        ...
        const email = JSON.parse(localStorage.getItem("data")).email;
        console.log(email);
        const res = axiosInstance.get(`youtube/playlist/?email=${email}`, { withCredentials: true });
        toast.promise(res, {
            loading: "Loading",
